...
<div class="panel-set-up">
    ...
    <form
        ...
        <select name="widget"
            text-type="h5"
            text-weight="bold">
            <?php foreach (CHATBOTCOM_Admin::get_instance()->data->connections as $widget) { ?>
                <option value="<?= $widget->id.':'.esc_html($widget->name) ?>"><?= esc_html($widget->name) ?></option>
            <?php } ?>
        </select>

        <input type="hidden" name="email" value="<?= CHATBOTCOM_Admin::get_instance()->data->email ?>">
        <input type="hidden" name="access_token" value="<?= CHATBOTCOM_Admin::get_instance()->data->access_token ?>">
